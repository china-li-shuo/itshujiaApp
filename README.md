...
通用书籍阅读APP，[一闪笔记](https://github.com/china-li-shuo/itshujiaApp) 的 `uni-app` 实现版本，支持多端分发，编译生成 Android 和 iOS 手机APP以及各平台的小程序。

使用`一闪笔记App`，您可以轻松地将您的电子书、文档手册和在线书籍分发到各个小程序平台，以及编译生成手机APP发布到各大手机软件市场供用户下载和使用。

# 如果想要联系我，可以关注我的公众号【一闪笔记】

![一闪笔记](static/images/wechat.png)

## APP 体验

目前 `一闪笔记App` 已经打包编译生成的 Android 和 iOS 手机APP，您可以直接下载体验和感受下。

APP下载体验地址： https://www.itshujia.com

## 设计开发

`一闪笔记App`的设计开发跟微信小程序[一闪笔记App](https://github.com/china-li-shuo/itshujia)几乎没有两样，如果有不一样的地方，
那就是从微信小程序的原生开发迁移到了使用`uni-app`的vue开发上来。

`一闪笔记App`中对HTML内容的渲染，经过多次对性能和渲染效果体验的对比，最终没有采用网上开源的第三方渲染组件，直接使用了`uni-app`自带的`rich-text`组件，
而将`一闪笔记App`编译成各平台的小程序的时候，同样也是用了各小程序平台的`rich-text`组件。
...
- 强大：尽管`一闪笔记App`使用的是混合开发，但是编译生成的手机App以及各平台的小程序，性能和体验也没见得真比原生的差


## 二次开发

可能您对现有程序的功能和主题色调不是很满意，或者有自己的一套UI设计方案，那么，您可以对`一闪笔记App`进行二次开发，如果您对`Vue`熟悉。
...
> 注意： 一闪笔记App 的安装部署，不是使用源码进行部署的，而是用编译好了的可执行程序进行部署
